...
class Indicators:
    ...
    def corr_data(self):
        ...
        corr_matrix = self.data.data.iloc[:, 1:].corr()
        ...
        for i in range(len(corr_matrix)):
            for j in range(len(corr_matrix)):
                plt.text(j + 0.5, i + 0.5, '{:.1f}'.format(corr_matrix.iloc[i, j]),
    ...
    def components_project(self):
        ...
        plt.scatter(self.pca2.components_[0], self.pca2.components_[1],
                    color='skyblue', edgecolor='black', s=100,
                    alpha=0.7)

        # Set labels for axes
        plt.xlabel('Principal Component 1 (36.5% of variance)', fontsize=15, fontweight="bold")
        plt.ylabel('Principal Component 2 (16.3% of variance)', fontsize=15, fontweight="bold")

        # Annotate each component
        for i, txt in enumerate(self.data.data.columns):
            plt.annotate(txt, (self.pca2.components_[0][i],
                               self.pca2.components_[1][i]), fontsize=10)
    ...
    def project_2D(self):
        ...
        components = pd.DataFrame(pca.components_.T, index=pd.DataFrame(self.data.data).columns,
                                  columns=['PCA1', 'PCA2'])
        ...
        plt.xlabel('First Dimension (36.5% of variance)', fontsize=15, fontweight="bold")
        plt.ylabel('Second Dimension (16.3% of variance)', fontsize=15, fontweight="bold")
        plt.ylim(10, -10)
        plt.xlim(10, -10)
        ...
        # Reference lines
        ax2.hlines(0, -0.4, 0.4, linestyles='dotted', colors='gray')
        ax2.vlines(0, -0.4, 0.4, linestyles='dotted', colors='gray')
        ...
        for a, i in enumerate(components.index):
            ax2.arrow(0, 0, components['PCA1'][a], -components['PCA2'][a],
                      alpha=0.5, facecolor='black', head_width=0.01)
            ax2.annotate(i, (components['PCA1'][a] * offset,
                             -components['PCA2'][a] * offset), color='black')

        # Save the figure
        plt.tight_layout()
        plt.savefig("figs/2D_projection.pdf")
